...
2. **Rolling refresh (improvement).** A better route appearing elsewhere is *not* locally detectable, so a hash-staggered 5–10% of agents re-query per tick, refreshing everyone within ~10–20 ticks without synchronized recomputation.
...
Point-to-point query p99 < 20μs at 10⁵-node graphs under the live metric; full customization < 10ms and typical partial customization < 1ms for 24 metrics; simulation speed ≥ 95% at 400k population on the benchmark save; corridor flow oscillation amplitude reduced ≥ 5× versus vanilla under the synchronized-demand stress test; zero increase in unreachable-trip failure events versus vanilla on identical saves.
